...
public class ShelfDetailForm extends AbstractInventoryDetailForm<LimsVO> {
	private static Logger		log	= LoggerFactory.getLogger(ShelfDetailForm.class);
	 ...
	 * @param node 
	 */
	public ShelfDetailForm(String id, FeedbackPanel feedBackPanel, WebMarkupContainer detailContainer, AbstractContainerForm<LimsVO> containerForm, InventoryLinkTree tree, DefaultMutableTreeNode node) {
		super(id, feedBackPanel, detailContainer, containerForm, tree, node);
	}

	public void initialiseDetailForm() {
		idTxtFld = new TextField<String>("invShelf.id");
		nameTxtFld = new TextField<String>("invShelf.name");
		capacityTxtFld = new TextField<Integer>("invShelf.capacity"){
		...
		capacityTxtFld.add(new AjaxFormComponentUpdatingBehavior("onChange") {
			...
			protected void onUpdate(AjaxRequestTarget target) {
				Integer capacity = containerForm.getModelObject().getInvShelf().getCapacity();
				containerForm.getModelObject().getInvShelf().setAvailable(capacity);
				target.add(availableTxtFld);
			}
		});
		availableTxtFld = new TextField<Integer>("invShelf.available");
		availableTxtFld.setEnabled(false);
		descriptionTxtAreaFld = new TextArea<String>("invShelf.description");
		...
		deleteButton.setEnabled(containerForm.getModelObject().getInvShelf().getChildren().isEmpty());
	}
	...
	private void initInvFreezerDdc() {
		...
		*/
		...
		invTankDdc = new DropDownChoice<InvFreezer>("invShelf.invFreezer", (List<InvFreezer>) invFreezerList, choiceRenderer);
	}
	...
	protected void onSave(Form<LimsVO> containerForm, AjaxRequestTarget target) {
		if (containerForm.getModelObject().getInvShelf().getId() == null) {
			// Save
			iInventoryService.createInvShelf(containerForm.getModelObject());
			this.info("Shelf " + containerForm.getModelObject().getInvShelf().getName() + " was created successfully");
			processErrors(target);
			
			if(node != null) {
				DefaultMutableTreeNode newNode = new DefaultMutableTreeNode(containerForm.getModelObject().getInvShelf());
				node.add(newNode);
			...
			// Update
			iInventoryService.updateInvShelf(containerForm.getModelObject());
			this.info("Shelf " + containerForm.getModelObject().getInvShelf().getName() + " was updated successfully");
			processErrors(target);
		}
	...
	protected void onDeleteConfirmed(AjaxRequestTarget target) {
		iInventoryService.deleteInvShelf(containerForm.getModelObject());
		this.info("Shelf " + containerForm.getModelObject().getInvShelf().getName() + " was deleted successfully");
	...
	protected boolean canDelete() {
		return containerForm.getModelObject().getInvShelf().getChildren().isEmpty();
	}
}