...
public class BiospecimenModalDetailForm extends AbstractModalDetailForm<LimsVO> {
	...
	private TextField<Number>									purity280;
	private TextField<Number>							purity230;
	...
	public void initialiseDetailForm() {
		...
		amountLbl = new Label("biospecimen.amount", new Model<Number>() {
			...
			public Number getObject() {
				...
				Number amount = ((concentration.doubleValue() * quantity.doubleValue())/1000);
				return amount;
			}
		});
		...
		purity280 = new TextField<Number>("biospecimen.purity280");
		purity230 = new TextField<Number>("biospecimen.purity230");

		barcodedChkBox = new CheckBox("biospecimen.barcoded");
		barcodedChkBox.setVisible(true);
		barcodedChkBox.setEnabled(true);
	...
	private void initSampleTypeDdc() {
		...
		sampleTypeDdc.add(new AjaxFormComponentUpdatingBehavior("onChange") {
			
			
			@Override
			protected void onUpdate(AjaxRequestTarget target) {
				if(sampleTypeDdc.getModel().getObject().getName()=="Nucleic Acid"){
					purity230.setEnabled(false);
					purity280.setEnabled(false);
				}else{
					purity230.setEnabled(true);
					purity280.setEnabled(true);
				}				
			}
		});
	}
	...
	private void initUnitDdc() {
		...
		unitDdc.add(new AjaxFormComponentUpdatingBehavior("onChange"){
			private static final long	serialVersionUID	= 1L;

			@Override
			protected void onUpdate(AjaxRequestTarget target) {
				if (cpModel.getObject().getBiospecimen().getUnit().getName()=="mL"){
					concentrationTxtFld.setVisible(false);
				}else{
					concentrationTxtFld.setVisible(true);
				}
			}	
		});
	...
	private void addComponents() {
		...
		arkCrudContainerVo.getDetailPanelFormContainer().add(purity280);
		arkCrudContainerVo.getDetailPanelFormContainer().add(purity230);
